class Travel::ProductsController < Travel::ApplicationController
  def index
    @country = Travel::Country.find(params[:where][:country_id]) if params.fetch(:where, {}).fetch(:country_id, nil).present?
    @city = Travel::City.find(params[:where][:city_id]) if params.fetch(:where, {}).fetch(:city_id, nil).present?
